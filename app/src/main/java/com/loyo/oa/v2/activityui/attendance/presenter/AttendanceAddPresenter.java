...
import com.loyo.oa.v2.activityui.attendance.AttendanceAddActivity;
...
public class AttendanceAddPresenter implements AttendanceAddPresenterImpl{
    ...
    public AttendanceAddPresenter(AttendanceRecord mAttendanceRecord,Context mContext,AttendanceAddView crolView,Activity mActivity){
    ...
    public boolean checkAttendanceData(String reason,String address,int outKind,int state) {
        if (TextUtils.isEmpty(reason)) {
            if (state == AttendanceRecord.STATE_OVERWORK) {
                if (outKind == 2) {
                    crolView.toastMessage("加班原因不能为空");
                    return false;
                }
            } else if (state == AttendanceRecord.STATE_LEAVE_EARLY) {
                crolView.toastMessage("早退原因不能为空");
                return false;
            } else if (state == AttendanceRecord.STATE_BE_LATE) {
                crolView.toastMessage("迟到原因不能为空");
                return false;
            }
        }

        if (TextUtils.isEmpty(address)) {
            crolView.toastMessage("地址不能为空");
            return false;
        }
        return true;
    }
