...
public class OrderEstimateListAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        holder.tv_esttime.setText(DateTool.timet(mEstimateAdd.receivedAt + "", "yyyy.MM.dd HH:mm"));
        ...
        if (!isAdd) {
            ...
            if (mEstimateAdd.status == 1 || mEstimateAdd.status == 2 || mEstimateAdd.status == 4 || mEstimateAdd.status == 5 || mEstimateAdd.status == 0||!isAdd) {
                holder.ll_action.setVisibility(View.GONE);
